...
export const _fetchUserChainData = async (
  ...
  const isBNBPool = tokenAddress == '0x0000000000000000000000000000000000000000'
  ...
  try {
    ...
    if (!isBNBPool && underlyingTokenSupportsAllowance) {
    ...
    if (!isBNBPool) {
      const res = await batch(provider, ...[etherplexTokenContract.balanceOf(usersAddress)])
      values.token = res.token
      console.log('token: ', values.token)
    } else {
      // get BNB valance of current address 
      const bnb_balance = await provider.getBalance(usersAddress)
      values.token = {
        balanceOf: [bnb_balance],
        "balanceOf(address)": [bnb_balance]
      }
    }
    ...
    console.log('useUserChainValues: ', e)
